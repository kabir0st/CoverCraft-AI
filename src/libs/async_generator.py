...
class AsyncBookDescriptionGenerator:
    ...
    def display_progress(self, force: bool = False):
        ...
        with self.stats_lock:
            ...
            progress_text = (f"\r🚀 Progress: {self.stats.completed}/"
            ...
            if self.stats.latest_failure_reason:
                print(f"\n💥 Latest failure: "
                      f"{self.stats.latest_failure_reason}")
    ...
    async def process_batch_async(
        ...
        num_agents = min(self.max_workers, len(items))
        agents = [GeneratorAgent(self.api_key) for _ in range(num_agents)]

        # Create semaphore to limit concurrent operations
        # Use a smaller semaphore to respect rate limits
        effective_workers = min(self.max_workers,
                                self.requests_per_minute // 4)
        semaphore = asyncio.Semaphore(effective_workers)

        print(f"🔧 Using {effective_workers} concurrent workers "
              f"(rate-limit optimized)")
    ...
    def process_batch_threaded(
        ...
        def process_item_sync(
                item: Dict[str, Any]) -> Union[Dict[str, Any], None]:
    ...
    async def process_items(
        ...
        mode = 'Threading' if use_threading else 'Async'
        print(f"🔧 Processing mode: {mode}")

        # # Estimate completion time
        # estimated_time = (len(items) *
        #                   self.min_request_interval) / 60  # minutes
    ...
    async def save_results(self,
        ...
        print(f"\n💾 Saving {len(results)} successful results to "
              f"{output_file}...")
...
async def main():
    ...
    MAX_WORKERS = min(10,
                      (os.cpu_count() or 1))  # Conservative for rate limiting
    BATCH_SIZE = 15  # Smaller batches for rate-limited processing
    USE_THREADING = False
